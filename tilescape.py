...
class TileScape():
    ...
    def __init__(self, mirror=None, test=False, save=False, debug=False):
        ...
        self.mirror = mirror
        self.test = test  # run the script based on webcam images stored in the input_files folder.
        self.debug = debug  # export intermediate processing steps to check if/where something goes wrong.
        self.turn = 1
        self.turn_count = 0
        self.update_count = 0
        self.save = save  # save intermediate files of the board as a shape file.
    ...
    def get_image(self):
        ...
        """
        if self.test:
            filename = 'DMG_table%d.jpg' % self.turn  # use this line for Delta Management Game test files
            #filename = 'VRG_table%d.jpg' % self.turn  # use this line for Virtual River Game test files
